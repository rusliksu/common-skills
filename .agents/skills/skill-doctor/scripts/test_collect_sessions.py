...
class StreamingSessionReadTests(unittest.TestCase):
    ...
    def test_new_harnesses_parse_records_after_previous_file_limit(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            ...
            zcode_path = root / "model-io-session.jsonl"
            write_jsonl_past_previous_limit(
                pi_path,
                {"type": "session", "id": "pi-session", "cwd": "/tmp/repo"},
                [{
                    "type": "message",
                    "message": {
                        "role": "assistant",
                        "content": [{
                            ...
                            "name": "read",
                            "arguments": {
                                "path": "/tmp/.pi/agent/skills/late-skill/SKILL.md"
                            },
                        }],
                    },
                }],
            ...
            write_jsonl_past_previous_limit(
                ...
                [{
                    "type": "assistant",
                    "content": "late grok message",
                    "tool_calls": [{
                        "name": "read_file",
                        "arguments": (
                            '{"path": "/tmp/.grok/skills/late-skill/SKILL.md"}'
                        ),
                    }],
                }],
            )
                ...
                [{
                    "request": {
                        "body": {
                            "messages": [{
                                "role": "assistant",
                                "content": "late zcode message",
                                "tool_calls": [{
                                    "name": "read_file",
                                    "arguments": (
                                        '{"path": "/tmp/.zcode/skills/late-skill/SKILL.md"}'
                                    ),
                                }],
                            }]
                        }
            ...
            parsed_pi = parse_pi_session(pi_path, {"late-skill"}, False)
            parsed_grok = parse_grok_session(grok_path, {"late-skill"}, False)
            parsed_zcode = parse_zcode_session(zcode_path, {"late-skill"}, False)

            for parsed in (parsed_pi, parsed_grok, parsed_zcode):
                self.assertEqual(parsed[1]["tool_calls"], 1)
                self.assertEqual(parsed[3], ["late-skill"])
